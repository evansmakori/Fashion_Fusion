...
export default class extends Service<Env> {
  ...
  async validateAddress(address: any): Promise<any> {
    ...
    const hasRequiredFields = address.streetAddress && address.city && 
                             address.state && address.zipCode && address.country;
    
    if (!hasRequiredFields || address.zipCode === 'invalid') {
      return {
        valid: false,
  ...
  async createOrder(params: any): Promise<any> {
    ...
    if (!params.shippingAddress.streetAddress) {
  ...
  async getOrderStatus(params: any): Promise<any> {
    if (params.orderId === 'invalid_order') {
